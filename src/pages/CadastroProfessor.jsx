import { useState } from "react";
import Header from "../components/Header_aluno";
import Footer from "../components/Footer";
import { criarProfessor } from "../services/api";
import "./Cadastro.css";

// Formata os dígitos do CPF como 000.000.000-00 (apenas para exibição)
function formatarCpf(valor) {
  return valor
    .replace(/\D/g, "")
    .slice(0, 11)
    .replace(/(\d{3})(\d)/, "$1.$2")
    .replace(/(\d{3})(\d)/, "$1.$2")
    .replace(/(\d{3})(\d{1,2})$/, "$1-$2");
}

function CadastroProfessor() {
  const [nome, setNome] = useState("");
  const [cpf, setCpf] = useState("");
  const [senha, setSenha] = useState("");
  const [confirmarSenha, setConfirmarSenha] = useState("");
  const [carregando, setCarregando] = useState(false);
  const [mensagem, setMensagem] = useState(null); // { tipo: "erro" | "sucesso", texto }

  async function handleSubmit(e) {
    e.preventDefault();
    setMensagem(null);

    if (!nome.trim() || !cpf.trim() || !senha) {
      setMensagem({ tipo: "erro", texto: "Preencha nome, CPF e senha." });
      return;
    }
    if (senha !== confirmarSenha) {
      setMensagem({ tipo: "erro", texto: "As senhas não coincidem." });
      return;
    }

    // Envia apenas os dígitos do CPF (coluna é varchar(11) no banco)
    const cpfNumeros = cpf.replace(/\D/g, "");
    if (cpfNumeros.length !== 11) {
      setMensagem({ tipo: "erro", texto: "CPF deve conter 11 dígitos." });
      return;
    }

    // admin_login é uma FK para admin.id_admin (PK), então enviamos o ID do admin logado
    const adminId = localStorage.getItem("id_admin");
    if (!adminId) {
      setMensagem({ tipo: "erro", texto: "Sessão de administrador não encontrada. Faça login novamente." });
      return;
    }

    try {
      setCarregando(true);
      const data = await criarProfessor({
        nome: nome.trim(),
        cpf: cpfNumeros,
        senha,
        admin_login: Number(adminId),
      });
      setMensagem({ tipo: "sucesso", texto: `Professor "${data.nome}" cadastrado com sucesso!` });
      setNome("");
      setCpf("");
      setSenha("");
      setConfirmarSenha("");
    } catch (err) {
      const texto =
        err.response?.data?.message || err.response?.data || "Erro ao cadastrar professor.";
      setMensagem({ tipo: "erro", texto });
    } finally {
      setCarregando(false);
    }
  }

  return (
    <>
      <Header />

      <div className="cad-page">
        <form className="cad-card" onSubmit={handleSubmit} noValidate>
          <div className="cad-card-header">
            <h1 className="cad-card-title">Cadastro de Professor</h1>
          </div>

          <div className="cad-card-body">
            <div className="cad-field">
              <label className="cad-label" htmlFor="prof-nome">Nome</label>
              <input
                id="prof-nome"
                className="cad-input"
                type="text"
                placeholder="Nome do professor"
                value={nome}
                onChange={(e) => setNome(e.target.value)}
                required
              />
            </div>

            <div className="cad-field">
              <label className="cad-label" htmlFor="prof-cpf">CPF</label>
              <input
                id="prof-cpf"
                className="cad-input"
                type="text"
                inputMode="numeric"
                maxLength={14}
                placeholder="000.000.000-00"
                value={formatarCpf(cpf)}
                onChange={(e) => setCpf(e.target.value.replace(/\D/g, "").slice(0, 11))}
                required
              />
            </div>

            <div className="cad-field">
              <label className="cad-label" htmlFor="prof-senha">Senha</label>
              <input
                id="prof-senha"
                className="cad-input"
                type="password"
                placeholder="Senha"
                value={senha}
                onChange={(e) => setSenha(e.target.value)}
                required
              />
              <span className="cad-hint">
                Mín. 6 caracteres, com pelo menos 1 letra, 1 número e 1 especial (!@#$%).
              </span>
            </div>

            <div className="cad-field">
              <label className="cad-label" htmlFor="prof-confirmar">Confirmar senha</label>
              <input
                id="prof-confirmar"
                className="cad-input"
                type="password"
                placeholder="Repita a senha"
                value={confirmarSenha}
                onChange={(e) => setConfirmarSenha(e.target.value)}
                required
              />
            </div>

            {mensagem && (
              <p className={`cad-msg cad-msg--${mensagem.tipo}`}>{mensagem.texto}</p>
            )}

            <button type="submit" className="cad-btn" disabled={carregando}>
              {carregando ? "Cadastrando..." : "Cadastrar"}
            </button>
          </div>
        </form>
      </div>

      <Footer />
    </>
  );
}

export default CadastroProfessor;
